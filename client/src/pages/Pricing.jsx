  ...
  const handlePayment = async (plan) => {
    try {
      ...
      const rzp  = new window.Razorpay({
        ...
        amount: result.data.amount, currency: "INR",
        name: "DevoraUI",
        description: `${plan.name} - ${plan.aiCredits} AI Credits`,
        order_id: result.data.id,
        handler: async function (response) {
          const verifyResponse = await axios.post(`${ServerURL}/api/payment/verify`, {
...
@import url('https://fonts.googleapis.com/css2?family=DM+Sans:ital,opsz,wght@0,9..40,100..1000;1,9..40,100..1000&family=Syne:wght@400..800&display=swap');
        ...
        <div className='relative z-10 max-w-3xl mx-auto px-4 py-14 w-full'>
          ...
          >
            <TbChevronLeft size={18} /> Back to Home
          </motion.button>
                ...
                {plans.map((plan, i) => (
                  <motion.div key={i}
                    ...
                    <div className='flex items-center justify-between mb-5'>
                      ...
                      {plan.disabled && (
                        <FiLock size={14} className="text-white/20" />
                      )}
                    </div>

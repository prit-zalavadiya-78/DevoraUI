...
function PropsInput({props, setProps}){
    ...
    const addProps = () => {
        let trimmed = input.trim().split(/\s+|,\s*/).filter(Boolean);
    ...
    const removeProps = (propToRemove)=>{
        setProps(props.filter(p=>p !== propToRemove))
    };
        ...
        <div>
            <div className='flex flex-wrap gap-1.5 mb-2 min-h-[28px]'>
                ...
                <span key={p}
                    ...
                    {p}
                    <button onClick={()=>{removeProps(p)}} className="ml-0.5 opacity-60 hover:opacity-100 transition-opacity bg-transparent border-none cursor-pointer p-0 leading-none"
                    style={{ color: "#a78bfa" }}>
                    <TbX size={11}/>
                    </button>
                </span>
            ...
            />
              <button onClick={addProps} className="px-3 sm:px-4 py-2 rounded-xl text-sm font-semibold border-none cursor-pointer transition-all whitespace-nowrap"
                style={{ background: "rgba(167,139,250,0.15)", color: "#a78bfa", border: "1px solid rgba(167,139,250,0.25)" }}>
                 Add
              </button>
            </div>
...
function AddComponentForm() {
    ...
    const handleSave = async ()=>{
        setSaving(true);
        try{
            const res = await axios.post(`${ServerURL}/api/component/save`, {name, code, props}, {withCredentials: true});
            console.log(res);
    ...
    const handlePublish = async () => {
        if(!savedId || publishing || isPublished) return;
        setPublishing(true);
        try {
            const res = await axios.post(`${ServerURL}/api/component/publish`, {componentId:savedId}, {withCredentials: true});
            setIsPublished(true);
            setPublishing(false);
  ...
  return (
    <div className='px-4 sm:px-6 lg:px-8 py-5 sm:py-6 max-w-3xl w-full mx-auto'>
        ...
        <div className='p-3.5 sm:p-4 rounded-2xl border border-white/[0.07] bg-white/[0.02] space-y-2'>
            ...
            type="text" id='name' placeholder='e.g. "PricingCard", "HeroSection"'
        ...
        <div className='rounded-2xl border border-white/[0.07] bg-white/[0.02] overflow-hidden'>
             <div className='flex items-center justify-between px-3.5 sm:px-4 py-3 border-b border-white/[0.06]'>
                <label className='text-xs font-semibold text-white/50 uppercase tracking-wider'>Component Code</label>
        ...
        <div className='flex items-center gap-2 sm:gap-3 flex-wrap pt-1'>
            <motion.button whileTap={{ scale: 0.97 }}
                ...
                onClick={handleSave}
                className="flex items-center gap-2 px-4 py-2.5 sm:px-5 rounded-xl text-sm font-semibold disabled:opacity-40 disabled:cursor-not-allowed transition-all border-none cursor-pointer"
                style={{
...
const AdminDashboard = () => {
    ...
    const SidebarContent = ()=>(
        <>
            ...
            <button onClick={() => setSidebarOpen(false)} className='ml-auto md:hidden bg-transparent border-none cursor-pointer p-1.5 rounded-lg text-white/40 hover:text-white/70 transition-colors'>
            ...
            <button onClick={handleLogout} className='w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-red-400/70 hover:text-red-400 hover:bg-red-500/[0.06] transition-all cursor-pointer bg-transparent border-none text-left'>
                <LuLogOut size={18} />
                <span>Logout</span>
            </button>
            </div>
    ...
    return (
        <div className="min-h-screen bg-[#030b0d] text-white flex overflow-hidden" style={{fontFamily:"'DM Sans', 'Poppins', sans-serif"}}>
                            ...
                            <motion.div
                                ...
                                <div className='flex items-start sm:items-center justify-between mb-4 sm:mb-5 gap-2'>
                                    <div className='min-w-0'>
                                        ...
                                        <p className="text-white/40 text-xs mt-0.5">Date-wise breakdown</p>
                                    </div>
                                    <span className='text-[10px] font-semibold px-2 sm:px-2.5 py-1 rounded-full bg-[#a78bfa]/10 text-[#a78bfa] border border-[#a78bfa]/20 flex-shrink-0'>Last 12 days</span>
                                </div>
                                {chartData?.length === 0 ? (
